      ...
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x" v-for="(attr,index) in searchParams.props" :key=index>{{attr.split(":")[1]}}<i @click="removeProps(index)">x</i></li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    trademarkInfo(trandmark){
      this.searchParams.trademark = `${trandmark.tmId}:${trandmark.tmName}`
      this.getGoodList()
    },
    removeTrademark(){
    ...
    attrInfo(attr,attr1){
      let props = `${attr.attrId}:${attr1}:${attr.attrName}`
    ...
    changeOrder(flag){
      ...
      if(flag === orginFlag){
        this.searchParams.order = `${flag}:${orginOrder === 'asc'? 'desc': 'asc'}`
      }else{
